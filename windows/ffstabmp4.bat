...
for %%A IN ("*.mp4") DO (
	...
	ffmpeg -i "%%A" -y -vf vidstabtransform=input=vidstab_vectors.trf -c:v h264_qsv -global_quality 27 -look_ahead 1 -g 15 -bf 2 -c:a copy "vidstab\%%A"
)